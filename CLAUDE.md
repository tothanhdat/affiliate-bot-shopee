...
**Nguồn affiliate — CHƯA CHỐT, đang cần quyết định lại (2026-08-17)**: Accesstrade ngừng duyệt campaign Shopee (Lazada không bị ảnh hưởng kỹ thuật). Từng ưu tiên Shopee Direct (`affiliate.shopee.vn`, đã nộp đơn KOC Affiliate), nhưng **Shopee đã trả lời chính thức qua email: Open API chỉ cấp cho nhóm KOL/KOC "có nhân viên hỗ trợ trực tiếp" (đối tác cấp doanh nghiệp/MCN), KHÔNG cấp cho tài khoản KOC cá nhân** — tức `ShopeeAffiliateProvider` **không làm được** cho tài khoản hiện tại, đây là kết luận cuối chứ không phải "đang chờ" nữa. Đừng code `ShopeeAffiliateProvider` hay tiếp tục hỏi Shopee về Open API cho tài khoản cá nhân. Xem `spec_bot_ap_ma_shopee.md` mục 9 để biết các hướng thay thế đang cân nhắc (Accesstrade cho Lazada, network khác như Involve Asia, đăng ký dạng MCN, hoặc Custom Link thủ công) — chưa hướng nào được chốt, hỏi user trước khi tự chọn 1 hướng để code theo. `AccesstradeProvider` vẫn giữ nguyên trong code (đã test kỹ, xem chi tiết bên dưới) làm phương án dự phòng, không xoá.
...
src/core/                    Core Service — khong biet gi ve Telegram/HTTP
  ...
  affiliateProvider.ts        interface AffiliateProvider (createAffiliateLink + getPromotions, ca 2 deu nhan merchant; createAffiliateLink co them itemId optional cho case nhu TikTok Shop) — diem noi de doi/them nguon affiliate khac Accesstrade
  providers/
